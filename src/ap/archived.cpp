...
namespace AP
{
    ...
    SgInitializedName *get_array_name_from_ref(SgPntrArrRefExp *array_ref, bool debug = false, int indent = 0)
    {
        ...
        if (debug)
        {
            const std::string indent_str = AP::get_indent(indent);
            const std::string inner_indent_str = AP::get_indent(1 + indent);
            std::cout << indent_str << "array_name: " << AP::to_string(array_name) << std::endl;
            std::cout << indent_str << "array: " << AP::to_string(array_name_exp) << std::endl;
            std::cout << indent_str << "subscript:" << std::endl;
            for (SgExpression *s : *array_ref_subscripts)
            {
                std::cout << inner_indent_str << AP::to_string(s) << std::endl;
            }
        }
        return array_name;
    }
    ...
    void for_stmt_ancestor_visitor(SgForStatement *from, SgScopeStatement *end, std::function<bool(SgForStatement *)> worker, bool debug)
    {
        ...
        if (debug)
        {
            std::cout << "ancestor traversal: " << AP::to_string(from) << std::endl;
        }
        ...
        while (cur_node != end)
        {
            if (debug)
            {
                std::cout << "  " << AP::to_string(cur_node) << std::endl;
            }
    ...
    SgInitializedName *is_loop_analyzable(SgNode *for_loop_node, bool debug = false, bool verbose = false)
    {
        ...
        if (debug)
        {
            std::cout << AP::to_string(for_loop_node) << std::endl;
        ...
        if (debug)
        {
            const std::string indent = AP::get_indent(1);
        ...
        if (is_canonical)
        {
            if (debug)
            {
                std::cout << AP::get_indent(1) << "is_canonical=True" << std::endl;
            }
            ...
            if (is_increment_step_constexpr)
            {
                if (debug)
                {
                    const std::string indent = AP::get_indent(2);
                ...
                if (is_induction_variable_unmodified)
                {
                    if (debug)
                    {
                        std::cout << AP::get_indent(3) << "is_induction_variable_unmodified=true" << std::endl;
                    }

                    std::cout << "Analyzable! " << AP::to_string(for_loop_node) << " with " << AP::to_string(ivar) << std::endl;
                    return ivar;
                }
            }
        }

        std::cout << "Not analyzable! " << AP::to_string(for_loop_node) << std::endl;
        return nullptr;
    }
        ...
        bool debug = false, int indent = 0)
    {
        ...
        if (debug)
        {
            std::cout << AP::get_indent(indent + 1) << "Name Matching.." << std::endl;
        }
        ...
        if (debug)
        {
            std::cout << AP::get_indent(indent + 1) << "Common Ancestor.." << std::endl;
            std::cout << AP::get_indent(indent + 2) << AP::to_string(ancestor_for_stmt) << std::endl;
        }
        ...
        if (debug)
        {
            std::cout << AP::get_indent(indent + 1) << "Outer for_stmts=" << outer_for_stmts.size() << ".." << std::endl;
            for (SgForStatement *for_stmt : outer_for_stmts)
            {
                std::cout << AP::get_indent(indent + 2) << AP::to_string(for_stmt) << std::endl;
            }
        }
                                                                   ...
                                                                   bool debug = false)
    {
        ...
        if (debug)
        {
            ...
            std::cout << "In scope: " << AP::to_string(scope_stmt) << std::endl;
        ...
        for (auto w_array_ref_it = write_array_refs.begin(); w_array_ref_it != write_array_refs.end(); ++w_array_ref_it)
        {
            ...
            if (debug)
            {
                std::cout << "Analyzing Write " << AP::to_string(w_array_ref) << std::endl;
            ...
            for (auto target_w_array_ref_it = w_array_ref_it + 1; target_w_array_ref_it != write_array_refs.end(); ++target_w_array_ref_it)
            {
                ...
                if (debug)
                {
                    std::cout << AP::get_indent(1) << "Write Target " << AP::to_string(target_w_array_ref) << std::endl;
                }
                ...
                if (std::optional<RawDDTP> res = is_potential_dependence_target_pair(w_array_ref, target_w_array_ref, scope_stmt, debug, 2))
                {
                    if (std::optional<DDTP> ddtp_opt = formulate_ddtp(*res, analyzable_loops))
                    {
                        if (debug)
                        {
                            std::cout << AP::get_indent(1) << *ddtp_opt << std::endl;
                        }
            ...
            for (SgPntrArrRefExp *target_r_array_ref : read_array_refs)
            {
                if (debug)
                {
                    std::cout << AP::get_indent(1) << "Read Target " << AP::to_string(target_r_array_ref) << std::endl;
                }
    ...
    void process_function_body(SgFunctionDefinition *defn, bool debug)
    {
        ...
        std::cout << AP::to_string(defn) << std::endl;
